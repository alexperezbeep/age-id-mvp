...
# 3. Step 2: Generalizable Visual Filters (Maintainer "Guessing" Logic)
# This uses surface-level traits to lock the correct logistics era
selected_profile = "[Not Sure]"

if selected_class != "[Auto-Detect from Silhouette]":
    st.subheader("Step 2: Describe Physical Profile")
    
    # Universal mapping of "Surface Info" to Technical Context
    profiles = {
...
# 4. Step 3: Image Capture & Analysis
    ...
    if st.button("🚀 EXECUTE LOGISTICS LOCK"):
        ...
        if not api_key:
            ...
            with st.spinner("Locking Context & Searching T.O.s..."):
                # The prompt now instructs the AI to use the visual profile to find the era
                prompt = f"""
                Maintainer Context: {final_context}
                
                Identify the part in the image. If the profile is 'Legacy', prioritize 1980s-1990s 
                equipment like the Davey Compressor H-1 (NSN 4520-01-056-4269). 
                If 'Modern', prioritize NGH-1 or MH-1 models.
